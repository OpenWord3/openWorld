	...
	foreach($tous_utilisateur as $res){
		...
		if(!empty($recherche)){
			...
			foreach($recherche as $res){
				...
				$rech[] = "<ol><h4><a href=" . $res->guid . " target=_blank><i class='fa fa-compass'></i> " . $res->post_title . "</a></h4></n>" . $res->guid . " <i class='fa fa-caret-down'></i><br>" . $post_date[0] . "<span>" . htmlentities($post_content[0]) . "<br>" . htmlentities($post_content[1]) . "</span></ol>";
			}			
		}	
	}
