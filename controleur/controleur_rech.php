	...
	foreach($tous_utilisateur as $res){
		...
		if(!empty($recherche)){
			...
			foreach($recherche as $res){
				...
				$rech[] = "<ol><h3><a href=" . $res->guid . " target=_blank><i class='fa fa-compass'></i> " . $res->post_title . "</a></h3></n>" . $res->guid . "<i class='fa fa-caret-down'></i><br>" . strftime("%Y-%m-%d", strtotime($post_date)) . substr($res->post_content, 10) . "</ol>";
			}			
		}	
	}
